...
async function sendWhatsAppInteractive(
  ...
  try {
    ...
    if (!res.ok) {
      const err = await res.text();
      console.error("[Cloud API] Erro ao enviar interativo:", res.status, err);
      return false;
    }

    return true;
  } catch (e) {
    console.error("[Cloud API] Erro ao enviar interativo:", e.message);
    return false;
  }
}
  ...
  for (const entry of entries) {
    ...
    for (const ch of changes) {
      ...
      if (isDynamoDBEnabled()) {
        ...
        if (!clientConfig) {
          console.warn(
            "[Cloud API] phone_number_id/business_account_id não encontrado no DynamoDB:",
            phoneNumberId,
            businessAccountId,
          );
          continue;
