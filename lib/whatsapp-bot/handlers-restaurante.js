...
/** Menu principal: Cardápio, Ver Status, Falar com Atendente (2 seções). */
...
/** Lista de bebidas em List Message */
function sendBebidasList(nomeRestaurante, bebidasDisp) {
  const rows = (bebidasDisp || []).slice(0, 10).map((b, i) => {
  ...
  return {
    interactive: {
      ...
      action: {
        button: "Ver Bebidas",
        sections: [{ title: "Nossas Bebidas", rows }],
      },
    },
  };
}
...
async function handleMessageRestaurante(from, text, config) {
  try {
    ...
    // Voltar: reseta para menu principal
    if (querVoltar(text)) {
      await clearConversa(config.phone_number_id, from, conversa);
      const interactive = sendMenuPrincipal(nomeRestaurante);
      console.log("2. Preparei menu principal (voltar)");
      return interactive;
    }

    // Mapear intenções: IDs exatos e fallback
    const isCardapio =
    ...
    if (isVerStatus && typeof config.getOrderStatus === "function") {
      ...
      console.log("2. Preparei resposta de status");
      return { reply: statusMsg };
    }

    // Falar com Atendente: manual_mode + prioridade
    if (isAtendente) {
      ...
      console.log("[PRIORIDADE] manual_mode=true para", from);
      console.log("2. Preparei confirmação de atendente");
      return {
        reply:
    ...
    if (isResumo) {
      ...
      if ((conversa.pedido?.itens || []).length === 0) {
        return {
          reply:
            resumo +
            "\n\nDigite *Cardápio* ou toque em Ver Opções para fazer um pedido.",
        };
      }
      return { reply: resumo };
    }

    // Estado AGUARDANDO_QUANTIDADE: usuário digitou a quantidade do item
    if (
      conversa.estado === ESTADO.AGUARDANDO_QUANTIDADE &&
      conversa.pedido?.tipoSelecionado
    ) {
      const qtd = parseInt(String(text).replace(/\D/g, ""), 10);
      if (!isNaN(qtd) && qtd >= 1 && qtd <= 99) {
        ...
        console.log("2. Quantidade adicionada, perguntando se quer mais");
        return {
          interactive: {
            type: "button",
            body: { text: `${resumo}\n\n${pergunta}` },
      ...
      return {
        reply: `Por favor, digite um número de 1 a 99 (quantidade de *${conversa.pedido.tipoSelecionado}*).`,
      };
    }

    // Estado PERGUNTAR_MAIS_ITEM: Sim -> cardápio; Não -> upsell batata
    if (conversa.estado === ESTADO.PERGUNTAR_MAIS_ITEM) {
      const resp =
        textNorm === "sim" || textNorm === "adicionar_sim" || textNorm === "s";
      const respNao =
        textNorm === "nao" ||
        textNorm === "não" ||
      ...
      return {
        reply:
          "Responda *Sim* para adicionar outro lanche ou *Não* para continuar.",
      };
    }

    // Estado OFFER_UPSELL: batata Sim/Não
      ...
      if (respNao) {
        ...
        return {
          interactive: {
            type: "button",
            body: {
              text: `É *delivery* ou *retirada no restaurante*?`,
            },
            action: { buttons: [btDelivery, btRetirar] },
          },
        };
      }
      return {
        reply: "Responda *Sim* para outra bebida ou *Não* para continuar.",
      };
    }

    // Estado TIPO_PEDIDO: Delivery ou Retirar
    if (conversa.estado === ESTADO.TIPO_PEDIDO) {
      ...
      if (ehRetirar) {
        ...
        if (resultado.success) {
          const msgFinal =
            resultado.reply +
            "\n\n✅ *Pedido confirmado!* Você pode consultar o status a qualquer momento no menu inicial.";
          return { reply: msgFinal };
        }
        return { reply: resultado.reply || "❌ Erro ao processar pedido." };
      }
      if (ehDelivery) {
        ...
        await saveConversa(config.phone_number_id, from, conversa);
        return {
          reply: `Perfeito! Para qual endereço devemos entregar? (Digite o endereço completo: rua, número, bairro)`,
        };
      }
      return {
        reply:
          "Escolha *Delivery* (entrega) ou *Retirar* (comer no restaurante).",
      };
    }

    // Estado ESCOLHER_BEBIDA: usuário selecionou bebida -> pergunta quantidade
    if (conversa.estado === ESTADO.ESCOLHER_BEBIDA) {
      ...
      if (
        ...
        await saveConversa(config.phone_number_id, from, conversa);
        return {
          reply: `Ótima escolha! 🥤 *${itemMenu.name}*\n\nQuantas unidades você deseja? (Digite o número)`,
        };
      }
    }

    // Estado ENDERECO_DELIVERY: usuário enviou o endereço -> criar pedido
    if (conversa.estado === ESTADO.ENDERECO_DELIVERY) {
      const endereco = (text || "").trim();
      if (!endereco || endereco.length < 5) {
        return {
          reply:
            "Por favor, digite o endereço completo da entrega (rua, número, bairro, ponto de referência).",
        };
      }
      conversa.pedido.endereco = endereco;
      conversa.pedido.telefone = String(from);
      conversa.pedido.nome = conversa.pedido.nome || "Cliente WhatsApp";
      conversa.pedido.metodoPagamento =
        conversa.pedido.metodoPagamento || "Não especificado";
      ...
      if (resultado.success) {
        const msgFinal =
          resultado.reply +
          "\n\n✅ *Pedido confirmado!* Você pode consultar o status a qualquer momento no menu inicial.";
        console.log("2. Pedido criado com sucesso");
        return { reply: msgFinal };
      }
      console.log("2. Erro ao criar pedido:", resultado.error);
      return {
        reply:
    ...
    // Seleção de item do cardápio: pergunta quantidade
    const itemMenu = getItemById(text);
