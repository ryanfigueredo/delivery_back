...
async function handleMessageRestaurante(from, text, config) {
  try {
    ...
    // Mapear intenções: IDs exatos (ver_cardapio, ver_status, falar_atendente) e fallback
    const isCardapio =
    ...
    // Ver Status: consulta pedido (try/catch para não quebrar por tenant/Prisma)
    ...
    // Cardápio ou menu principal (oi, etc)
    if (isCardapio) {
