...
async function handleMessageRestaurante(from, text, config) {
  try {
    ...
    // Estado AGUARDANDO_QUANTIDADE: Voltar cancela escolha; número = quantidade
    if (
      ...
      return {
        reply: `Digite a quantidade de *${nomeItem}* (1 a 99) ou *Voltar* para escolher outro item.`,
      };
    }
    ...
    if (conversa.estado === ESTADO.ESCOLHER_BEBIDA) {
      ...
      if (
        ...
        await saveConversa(config.phone_number_id, from, conversa);
        return {
          reply: `Ótima escolha! 🥤 *${itemMenu.name}*\n\nQuantas unidades? (Digite o número ou *Voltar* para escolher outra)`,
        };
      }
    }
    ...
    if (itemMenu && itemMenu.available !== false) {
      ...
      return {
        reply: `Ótima escolha! 🍔 *${itemMenu.name}*\n\nQuantas unidades você deseja? (Digite o número ou *Voltar* para escolher outro)`,
      };
    }
