...
async function handleMessageRestaurante(from, text, config) {
  try {
    ...
    // Seleção de item do cardápio: inicia checkout (endereço)
    ...
    if (itemMenu && itemMenu.available !== false) {
      ...
      await saveConversa(config.phone_number_id, from, conversa);
      console.log("2. Item adicionado, aguardando endereço");
      return {
        reply: `Ótima escolha! 🍔 *${itemMenu.name}* adicionado.\n\nPara qual endereço devemos entregar? (Digite o endereço completo: rua, número, bairro)`,
      };
    }
