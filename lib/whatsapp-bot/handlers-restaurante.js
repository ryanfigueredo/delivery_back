...
const CATEGORY_ORDER = ["hamburguer", "bebida", "acompanhamento", "sobremesa"];

/** Cardápio em List Message: agrupado por categoria, máx 10 itens por seção (regra Meta). */
function sendCardapioList(nomeRestaurante, cartEnc, items) {
  ...
  for (const cat of CATEGORY_ORDER) {
    const rows = available
      ...
      .slice(0, META_LIST_MAX_ROWS_PER_SECTION)
      .map((i) => ({
        id: prefix(String(i.id)).slice(0, 200),
        title: (i.name || "").slice(0, 24),
        description: `R$ ${Number(i.price || 0)
          .toFixed(2)
          .replace(".", ",")}`,
      }));
    if (rows.length > 0) {
  ...
  // Categorias que não estão em CATEGORY_ORDER (ex: custom)
  const doneCats = new Set(CATEGORY_ORDER);
  const otherCats = [
    ...new Set(
      available.map((i) => (i.category || "").toLowerCase()).filter(Boolean)
    ),
  ].filter((c) => !doneCats.has(c));
    ...
    rows: [
      {
        id: "voltar_cardapio",
        title: "← Voltar",
        description: "Menu anterior",
      },
    ],
  });

  const totalItems = sections.reduce((s, sec) => s + sec.rows.length, 0) - 1;
  console.log(
    "Enviando cardápio (list) com " + totalItems + " itens por categoria"
  );
  return {
    interactive: {
      type: "list",
      body: {
        text: `*${nomeRestaurante}*\n\n🍔 *Faça seu pedido!*\n\nEscolha por categoria: Lanches, Bebidas, etc.`,
      },
