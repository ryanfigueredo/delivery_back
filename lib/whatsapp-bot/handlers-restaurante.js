...
async function fetchMenu(config) {
  try {
    ...
    if (!res.ok) {
      console.log(
        "[Restaurante] Falha na API, usando menu fixo agora (status:",
        res.status,
        ")"
      );
    ...
    return Array.isArray(items) && items.length > 0
      ? items
      : MENU_HARDCODED_EMERGENCIA;
  } catch (e) {
    console.log("[Restaurante] Falha na API, usando menu fixo agora");
    console.error("[Restaurante] Erro ao buscar menu:", e?.message || e);
    return MENU_HARDCODED_EMERGENCIA;
  }
}
