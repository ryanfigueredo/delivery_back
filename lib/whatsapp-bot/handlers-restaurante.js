...
function sendCardapioList(nomeRestaurante) {
...
async function handleMessageRestaurante(from, text, config) {
  try {
    ...
    // STATELESS: finish|CART — ignora banco, pergunta endereço imediatamente
    if (textRaw.startsWith("finish|")) {
      ...
      if (itens.length > 0) {
        ...
        return {
          reply:
            "Ótimo! Qual o endereço de entrega? Por favor, digite o seu endereço completo.\n\nOu digite *Retirar* para retirada no restaurante.",
        };
      }
    }
