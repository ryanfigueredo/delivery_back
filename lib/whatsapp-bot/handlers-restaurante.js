...
/** Cardápio em List Message: agrupado por categoria, máx 10 itens total (regra Meta). */
function sendCardapioList(nomeRestaurante, cartEnc, items) {
  ...
  // Limitar a apenas hamburgueres e bebidas (5 + 4 = 9 itens total)
  const hamburgueres = available
    .filter((i) => (i.category || "").toLowerCase() === "hamburguer")
    .slice(0, 5) // Máximo 5 hamburgueres
    .map((i) => ({
  ...
  if (hamburgueres.length > 0) {
    sections.push({
      title: CATEGORY_LABELS.hamburguer || "Lanches",
      rows: hamburgueres,
    });
  }

  const bebidas = available
    .filter((i) => (i.category || "").toLowerCase() === "bebida")
    .slice(0, 4) // Máximo 4 bebidas
    .map((i) => ({
      id: prefix(String(i.id)).slice(0, 200),
      title: (i.name || "").slice(0, 24),
      description: `R$ ${Number(i.price || 0)
  ...
  if (bebidas.length > 0) {
    sections.push({
      title: CATEGORY_LABELS.bebida || "Bebidas",
      rows: bebidas,
    });
  }

  // Removido: seção "Ações" e outras categorias para não exceder 10 itens
