...
/** Cardápio em List Message: 5 hambúrgueres + 4 bebidas (máx 9 itens, sem Ações). */
function sendCardapioList(nomeRestaurante, cartEnc) {
  ...
  const prefix = cartEnc ? (id) => `add_${id}|${cartEnc}` : (id) => id;
  const rowsHamb = hamburgueres.slice(0, 5).map((h, i) => ({
    id: prefix(`hamb_${i + 1}`).slice(0, 200),
    title: (h.name || "").slice(0, 24),
    description: `R$ ${Number(h.price || 0)
      .toFixed(2)
      .replace(".", ",")}`,
  }));
  const rowsBeb = bebidas.slice(0, 4).map((b, i) => ({
    id: prefix(`bev_${i + 1}`).slice(0, 200),
    title: (b.name || "").slice(0, 24),
    description: `R$ ${Number(b.price || 0)
      .toFixed(2)
      .replace(".", ",")}`,
  }));

  const sections = [
    { title: "Hambúrgueres", rows: rowsHamb },
    { title: "Bebidas", rows: rowsBeb },
  ];
  const totalItems = rowsHamb.length + rowsBeb.length;
...
function getItemById(itemId) {
  const id = String(itemId || "")
    ...
    .toLowerCase();
  const resolved =
    CARDAPIO_SHORT_IDS[id] || CARDAPIO_SHORT_IDS[id.replace("-", "_")] || id;
...
async function handleMessageRestaurante(from, text, config) {
  try {
    ...
    // STATELESS: addmore|CART — volta ao cardápio para adicionar mais (sem DynamoDB)
    if (textRaw.startsWith("addmore|")) {
      const payload = textRaw.slice(8).trim();
      const cartEnc = payload || "";
      return sendCardapioList(nomeRestaurante, cartEnc);
    }

    // STATELESS: add_ITEMID|CART — selecionou item para adicionar, mostra quantidade
    const addMatch = textRaw.match(/^add_(hamb_\d|bev_\d)\|(.+)$/);
    if (addMatch) {
      const [, shortId, cartEnc] = addMatch;
      const itemMenu = getItemById(shortId);
            ...
            interactive: {
              ...
              action: {
                buttons: [
                  {
                    type: "reply",
                    reply: { id: `addmore|${newCartEnc}`, title: "Sim" },
                  },
                  {
                    type: "reply",
                    reply: { id: `local|${newCartEnc}`, title: "Não" },
                  },
                ],
              },
            },
    ...
    // STATELESS: emcasa|CART — mostra opções de endereço (evita perder cart sem DynamoDB)
    if (textRaw.startsWith("emcasa|")) {
      ...
      if (itens.length > 0) {
        const cartEnc = encodeCart(itens);
        const safeAddr = (addr) =>
          `addr_${addr.replace(/\|/g, "-")}|${cartEnc}`.slice(0, 200);
        return {
          interactive: {
            type: "list",
            body: {
              text: "📍 *Escolha o endereço de entrega ou digite na próxima mensagem:*",
            },
            action: {
              button: "Ver endereços",
              sections: [
                {
                  title: "Endereços",
                  rows: [
                    {
                      id: safeAddr("Rua da Liberdade, 82"),
                      title: "Rua da Liberdade, 82",
                      description: "Entrega",
                    },
                    {
                      id: safeAddr("Rua da Liberdade"),
                      title: "Rua da Liberdade",
                      description: "Informar número",
                    },
                    {
                      id: `addr_type|${cartEnc}`.slice(0, 200),
                      title: "Outro - vou digitar",
                      description: "Informar endereço completo",
                    },
                  ],
                },
              ],
    ...
    // STATELESS: addr_ENDERECO|CART ou addr_type|CART
    if (textRaw.startsWith("addr_")) {
      const rest = textRaw.slice(5);
      const pipeIdx = rest.indexOf("|");
      const cartEnc = pipeIdx >= 0 ? rest.slice(pipeIdx + 1) : "";
      const addrPart = pipeIdx >= 0 ? rest.slice(0, pipeIdx) : rest;
      ...
      if (addrPart === "type") {
        ...
        if (itens.length > 0) {
          ...
          return {
            reply:
              "Por favor, digite seu endereço completo (rua, número, bairro).",
          };
        }
      } else {
        const endereco = addrPart.replace(/-/g, ",").trim();
    ...
    if (isVerStatus) {
      return {
        reply:
          "Sistema de consulta em manutenção, mas seu pedido está sendo processado!",
      };
    }
    if (isAtendente) {
