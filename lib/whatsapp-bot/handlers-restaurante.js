...
/** Cardápio em List Message: Hambúrgueres + Bebidas + Voltar. */
function sendCardapioList(nomeRestaurante) {
  ...
  const rowsHamb = hamburgueres.slice(0, 6).map((h) => ({
    id: (h.id || `item_${h.name}`).slice(0, 200),
    title: (h.name || "").slice(0, 24),
    description: `R$ ${Number(h.price || 0)
      .toFixed(2)
      .replace(".", ",")}`,
  }));
  const rowsBeb = bebidas.slice(0, 6).map((b) => ({
    id: (b.id || `bebida_${b.name}`).slice(0, 200),
