...
/** Lista de quantidade (1 a 5) + Voltar — evita erro ao digitar número. */
function sendQuantidadeList(nomeRestaurante, itemName, emoji) {
  const e = emoji || "🍔";
        ...
        sections: [
          {
            title: "Quantidade",
            rows: [
              { id: "qty_1", title: "1 unidade", description: "" },
              { id: "qty_2", title: "2 unidades", description: "" },
              { id: "qty_3", title: "3 unidades", description: "" },
              { id: "qty_4", title: "4 unidades", description: "" },
              { id: "qty_5", title: "5 unidades", description: "" },
            ],
          },
          {
            title: "Ações",
...
async function handleMessageRestaurante(from, text, config) {
  try {
    ...
    if (
      ...
      // Aceitar qty_voltar (botão) ou texto "voltar"
      if (
        querVoltar(text) ||
        textNorm === "voltar" ||
        textNorm === "qty_voltar"
      ...
      return sendQuantidadeList(nomeRestaurante, nomeItem, emoji);
    }
    ...
    if (conversa.estado === ESTADO.TIPO_PEDIDO) {
      ...
      if (ehRetirar) {
        ...
        if (resultado.success) {
          const msgFinal =
            resultado.reply +
            "\n\n✅ *Pedido confirmado!* Obrigado pela preferência. Use *Ver Status* no menu para acompanhar.";
      ...
      if (ehDelivery) {
        ...
        await saveConversa(config.phone_number_id, from, conversa);
        return {
          reply: `📍 *Endereço de entrega*\n\nDigite o endereço completo (rua, número, bairro).\n\nOu digite *Voltar* para alterar para retirada.`,
        };
      }
      return {
        reply:
    ...
    if (conversa.estado === ESTADO.ESCOLHER_BEBIDA) {
      ...
      if (
        ...
        conversa.estado = ESTADO.AGUARDANDO_QUANTIDADE;
        await saveConversa(config.phone_number_id, from, conversa);
        return sendQuantidadeList(nomeRestaurante, itemMenu.name, "🥤");
      }
    }

    // Estado ENDERECO_DELIVERY: endereço ou Voltar
    if (conversa.estado === ESTADO.ENDERECO_DELIVERY) {
      if (querVoltar(text) || textNorm === "voltar") {
        conversa.estado = ESTADO.TIPO_PEDIDO;
        await saveConversa(config.phone_number_id, from, conversa);
        const btDelivery = {
          type: "reply",
      ...
      if (!endereco || endereco.length < 5) {
        return {
          reply:
            "Digite o endereço completo (rua, número, bairro) ou *Voltar* para alterar.",
        };
      }
