...
const ESTADO = {
  INICIO: "inicio",
  CARDAPIO: "cardapio", // escolher hambúrguer
    ...
    switch (conversa.estado) {
      case ESTADO.INICIO: {
        ...
        if (textNorm === "cardapio" || textoLower === "1") {
          ...
          // Primeiro: mensagem para fazer o pedido + só opções de hambúrguer
          let msg = `*${nomeRestaurante}*\n\n`;
          msg += `📋 *Faça seu pedido!*\n\n`;
          msg += `Escolha seu *hambúrguer*:\n`;
          msg += `━━━━━━━━━━━━━━━━━━━━\n\n`;
          let num = 1;
          hamburgueresDisp.forEach((h) => {
            msg += `${num}. ${h.name} — R$ ${Number(h.price)
              .toFixed(2)
              .replace(".", ",")}\n`;
            num++;
          });
          msg += `\n━━━━━━━━━━━━━━━━━━━━\n`;
          msg += `Digite o *número* ou *nome* do hambúrguer.\n\n`;
          msg += `⬅️ *VOLTAR* — voltar`;
          return { reply: msg };
        }
        if (textNorm === "resumo") {
      ...
      case ESTADO.CARDAPIO: {
        if (querVoltar(text)) {
          conversa.estado = ESTADO.INICIO;
          return {
            reply: `*${nomeRestaurante}*\n\nFaça seu pedido! Digite *Cardápio* ou toque no botão 📋 Cardápio.`,
          };
        }
        let item = null;
        ...
        if (!item) {
          return {
            reply:
              "❌ Opção inválida. Digite o número do hambúrguer.\n\n⬅️ *VOLTAR*",
          };
        }
      ...
      case ESTADO.QUANTIDADE_HAMBURGUER: {
        ...
        let msgBebida = `✅ ${qtd}x ${nomeItem} adicionado!\n\n`;
        msgBebida += `Agora escolha sua *bebida*:\n`;
        msgBebida += `━━━━━━━━━━━━━━━━━━━━\n\n`;
      ...
      case ESTADO.ESCOLHER_BEBIDA: {
        if (querVoltar(text)) {
          conversa.estado = ESTADO.QUANTIDADE_HAMBURGUER;
          return {
            reply: "Digite a quantidade do hambúrguer (1 a 10).\n\n⬅️ *VOLTAR*",
          };
        }
        if (
          textoLower === "0" ||
