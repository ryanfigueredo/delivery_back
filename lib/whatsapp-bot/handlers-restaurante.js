...
const MENU_HARDCODED_EMERGENCIA = [
  ...
  { id: "refrigerante_guarana", name: "Guaraná", price: 5, available: true },
  { id: "refrigerante_fanta", name: "Fanta", price: 5, available: true },
  { id: "suco_laranja", name: "Suco de Laranja", price: 6, available: true },
  { id: "suco_maracuja", name: "Suco de Maracujá", price: 6, available: true },
  { id: "suco_limao", name: "Suco de Limão", price: 6, available: true },
  { id: "suco_abacaxi", name: "Suco de Abacaxi", price: 6, available: true },
...
async function fetchMenu(config) {
  ...
  const urlFinal = `${base.replace(/\/$/, "")}/api/bot/menu/public`;
  const apiKey = config.tenant_api_key || "";
  const headers = { "X-API-Key": apiKey, "Content-Type": "application/json" };
  console.log("[Restaurante] Tentando buscar menu em:", urlFinal);
  console.log(
    "[Restaurante] Headers enviados: X-API-Key=",
    apiKey ? `${apiKey.slice(0, 8)}...` : "(vazio)"
  );
  try {
    ...
    if (res.status === 401) {
      console.log(
        "[Restaurante] 401 na API de menu - usando menu hardcoded de emergência"
      );
