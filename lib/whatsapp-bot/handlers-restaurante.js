  ...
  switch (conversa.estado) {
    case ESTADO.INICIO: {
      if (
        ...
        menuTexto += `\n*Bebidas:*\n`;
        bebidas.forEach((b, i) => {
          menuTexto += `${hamburgueres.length + i + 1}. ${b.name} - R$ ${Number(
            b.price,
          )
            .toFixed(2)
            .replace(".", ",")}\n`;
        });
        menuTexto += `\nDigite o *NÚMERO* da opção ou descreva seu pedido!\n\n⬅️ *VOLTAR* para voltar`;
        return { reply: menuTexto };
      }
      if (textoLower === "2" || textoLower === "resumo") {
        return {
          reply:
            conversa.pedido.itens.length > 0
    ...
    case ESTADO.CARDAPIO: {
      ...
      if (isNaN(escolhaNum) || escolhaNum < 1 || escolhaNum > items.length) {
        return {
          reply:
            "❌ Opção inválida. Digite o número do item.\n\n⬅️ *VOLTAR* para voltar",
        };
      }
      const item = items[escolhaNum - 1];
      if (!item || !itemDisponivel(item.id, estoque)) {
